...
const createRaf = (cb: (time: number) => void) => setTimeout(() => cb(Date.now()), 16) as unknown as number
...
export function applyBrowserPolyfills(win: PolyfillWindow) {
  ...
  if (!('IntersectionObserver' in g)) {
    ...
    /* eslint-disable no-undef */
    g.IntersectionObserver = NoopIntersectionObserver as unknown as typeof IntersectionObserver
    /* eslint-enable no-undef */
    ;(win as Record<string, unknown>).IntersectionObserver = NoopIntersectionObserver
  }
  ...
  /* eslint-disable no-undef */
  g.requestAnimationFrame = (win.requestAnimationFrame || createRaf) as typeof requestAnimationFrame
  g.cancelAnimationFrame = (win.cancelAnimationFrame || clearTimeout) as typeof cancelAnimationFrame
  /* eslint-enable no-undef */
  ;(win as Record<string, unknown>).requestAnimationFrame = g.requestAnimationFrame
  ;(win as Record<string, unknown>).cancelAnimationFrame = g.cancelAnimationFrame
}

export async function processHtmlFile(htmlFilePath: string, targetFiles: string[], componentsDir: string) {
